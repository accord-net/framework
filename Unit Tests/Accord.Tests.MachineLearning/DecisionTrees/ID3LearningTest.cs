...
namespace Accord.Tests.MachineLearning
{
    ...
    public class ID3LearningTest
    {
        ...
        [Test]
        public void learn_test_max_variables()
        {
            int[][] inputs =
            {
                new int[] { 0, 0 },
            ...
            Assert.AreEqual(1, tree.Root.Branches.AttributeIndex); // y
            Assert.AreEqual(2, tree.Root.Branches.Count);
            Assert.IsNull(tree.Root.Value);
            Assert.IsNull(tree.Root.Value);

            Assert.AreEqual(0.0, tree.Root.Branches[0].Value); // y = [0]
            Assert.AreEqual(1.0, tree.Root.Branches[1].Value); // y = [1]
