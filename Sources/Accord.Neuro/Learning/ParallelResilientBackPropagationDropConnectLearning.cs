...
//
// Copyright © Kory Becker, 2014
// kbecker@primaryobjects.com
// For details on Backpropagation + DropConnect, see http://cs.nyu.edu/~wanli/dropc/.
//
...
namespace Accord.Neuro.Learning
{
...
#if !NET35
    /// <summary>
    ///   Resilient Backpropagation + DropConnect learning algorithm.
    /// </summary>
    /// 
    ...
    /// 
    public class ParallelResilientBackpropagationDropConnectLearning : ISupervisedLearning, IDisposable
    {
        ...
        /// <summary>
        /// DropConnect threshold for visible weights. This value represents the chance a weight has of being reset to zero. A random number must exceed this value in order to reset the weight.
        /// Example: 0.80 = 20% chance.
        /// Default: 0.80
        /// </summary>
        public double DropConnectVisibleThreshold { get; set; }
        /// <summary>
        /// DropConnect threshold for hidden weights. This value represents the chance a weight has of being reset to zero. A random number must exceed this value in order to reset the weight.
        /// Example: 0.80 = 20% chance.
        /// Default: 0.50
        /// </summary>
        ...
        /// <summary>
        /// Function to calculate the new value for a weight that is being reset. By default, the weight will be reset to zero. This can be customized to reset the weight to a calculated value, based on some property of the neuron or its weight.
        /// Default: (neuron) => { return 0 }.
        /// </summary>
        public Func<Neuron, double> DropConnectResetMethod { get; set; }
        /// <summary>
        ...
        /// 
        public ParallelResilientBackpropagationDropConnectLearning(ActivationNetwork network)
        {
        ...
        private void UpdateNetwork()
        {
            ...
            for (int i = 0; i < weightsUpdates.Length; i++)
            {
                ...
                // The input layer will DropConnect 20%. Hidden layers 50%.
                double dropConnectThreshold = i == 0 ? DropConnectVisibleThreshold : DropConnectHiddenThreshold;
                ...
                for (int j = 0; j < layerWeightsUpdates.Length; j++)
                {
                    ...
                    for (int k = 0; k < neuronPreviousWeightDerivatives.Length; k++)
                    {
                        ...
                        // Randomly select if this weight should be dropped, as long as it holds a value greater than a specific value.
                        if (neuron.Weights[k] > DropConnectWeightThreshold && _random.NextDouble() > dropConnectThreshold)
                        {
                            // DropConnect this weight by setting it to zero.
                            neuron.Weights[k] = DropConnectResetMethod(neuron);
                        }
                    }
        ...
        /// </summary>
        /// 
        ~ParallelResilientBackpropagationDropConnectLearning()
        {
            Dispose(false);
        }
