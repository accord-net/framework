...
namespace Accord.Statistics.Filters
{
    ...
    public class Codification : Codification<string>, IAutoConfigurableFilter, ITransform<string[], double[]>
    {
        ...
        double[] ITransform<string[], double[]>.Transform(string[] input)
        {
            int total = 0;
            ...
            foreach (var col in Columns)
            {
                int[] indices = base.Transform(input);
        ...
        double[][] ITransform<string[], double[]>.Transform(string[][] input)
        {
            ...
            return Transform(input, Jagged.Zeros(input.Length, total));
        }
        ...
        public double[][] Transform(string[][] input, double[][] result)
        {
            for (int j = 0; j < input.Length; j++)
            {
                ...
                foreach (var col in Columns)
                {
                    int[] indices = col.Transform(input[j]);
                    for (int i = 0; i < indices.Length; i++)
                        result[j][k++] = indices[i];
                }
            }
