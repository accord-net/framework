...
#pragma warning restore 612, 618
    {
        ...
        /// </summary>
        /// 
        [Obsolete("Please use Decide() or Distances() instead.")]
        public int Classify(double[] input)
        {
            return Classes[Decide(input)].Number;
        }
        ...
        public override double[] Distances(double[] input, double[] result)
        {
            ...
            for (int i = 0; i < projectedMeans.Length; i++)
                result[i] = Accord.Math.Distance.SquareEuclidean(projection, ProjectionMeans[i]);
            return result;
        }
